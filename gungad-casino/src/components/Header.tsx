    ...
    <>
      ...
      >
        <div className="w-full px-3 sm:px-5 lg:px-6 py-2 flex items-center justify-between gap-2 min-w-0">
          ...
          {/* Right: badges + balance + lang + profile */}
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
                ...
                <div className="absolute right-0 top-full mt-2 w-48 bg-[#111116] border border-zinc-800 rounded-2xl shadow-2xl p-1.5 z-[300]">
                  ...
                  {Object.values(CURRENCIES).map((c) => (
                    <button
                      type="button"
                      key={c.code}
                      onClick={(e) => { e.stopPropagation(); soundFx.playClick(); onCurrencyChange(c.code); setCurrOpen(false); }}
            ...
            <div className="relative">
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); soundFx.playClick(); setLangOpen(v => !v); setCurrOpen(false); }}
                ...
                <div className="absolute right-0 top-full mt-2 w-40 bg-[#111116] border border-zinc-800 rounded-2xl shadow-2xl p-1.5 z-[300]">
                  {Object.values(LANGUAGES).map((l) => (
                    <button
                      type="button"
                      key={l.code}
                      onClick={(e) => { e.stopPropagation(); soundFx.playClick(); onLangChange(l.code); setLangOpen(false); }}
